...
from BaselinesModules import Resnet
...
class ShearLayer:
    def __init__(self, network_properties, device, batch_size, training_samples, in_size=64, file="ddsl_N128/", cluster=False, insample=True):
        ...
        torch.manual_seed(retrain)
        self.model = Resnet(input_dimension=(1 + 2 * self.N_Fourier_F) * 64 * 64, output_dimension=64 * 64, retrain=retrain, neurons=neurons, n_hidden_layers=layers).to(device)
...
class EquationModel:
    def __init__(self, network_properties, device, batch_size, which_data, training_samples=1024, insample=True):
        ...
        self.model = Resnet(input_dimension=(1 + 2 * self.N_Fourier_F) * res * res, output_dimension=res * res, retrain=retrain, neurons=neurons, n_hidden_layers=layers).to(device)
