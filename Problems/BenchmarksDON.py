...
class ShearLayer:
    def __init__(self, network_properties, device, batch_size, training_samples, in_size=64, file="ddsl_N128/", cluster=False, insample=True):
        ...
        trunk = FeedForwardNN(2, basis, layers=trunk_layers, neurons=trunk_neurons, retrain=retrain).to(device)

        self.model = DeepOnetNoBiasOrg(branch, trunk).to(device)

        training_inputs, training_outputs = self.get_data(folder + file, 1024, insample)
        ...
        branch_training_inputs = training_inputs[:, 0].unsqueeze(1)
        branch_validation_inputs = validation_inputs[:, 0].unsqueeze(1)
        branch_testing_inputs = testing_inputs[:, 0].unsqueeze(1)

        self.trunk_inputs = training_inputs[0, 1:].reshape(2, -1).permute(1, 0).to(device)
        print(self.trunk_inputs.shape)

        self.train_loader = DataLoader(TensorDataset(branch_training_inputs, training_outputs), batch_size=batch_size, shuffle=True)
        self.test_loader = DataLoader(TensorDataset(branch_validation_inputs, validation_outputs), batch_size=batch_size, shuffle=False)
        self.test_loader_2 = DataLoader(TensorDataset(branch_testing_inputs, testing_outputs), batch_size=batch_size, shuffle=False)

        if not insample:
            file = "data/Euler_Shear_0_05/"
            folder = "data/Euler_Shear/"

            testing_inputs, testing_outputs = self.get_data(folder + file, 1024, insample)
            ...
            testing_inputs = testing_inputs[:, 0].unsqueeze(1)
    ...
    def get_data(self, folder, n_samples, insample):
        ...
        input_data = np.zeros((n_samples, 1 + 2, self.in_size, self.in_size))
        output_data = np.zeros((n_samples, 1, self.in_size, self.in_size))

        grid = self.get_grid().permute(2, 0, 1)
        input_data[:, 1:] = grid
...
class WaveEquationDataset(Dataset):
    def __init__(self, which="training", nf=0, training_samples=1024, t=5, insample=True):
...
class EquationModel:
    def __init__(self, network_properties, device, batch_size, which_data, training_samples=1024, insample=True):
        ...
        if self.N_Fourier_F != 0:
            self.trunk_inputs = training_set.get_grid().reshape(-1, 2 * self.N_Fourier_F).to(device)
        else:
            self.trunk_inputs = training_set.get_grid().reshape(-1, 2).to(device)
